...
export class DbService {
  ...
  private debug = true;
        ...
        return new Promise(resolve => {
            ...
            openRequest.onupgradeneeded = event => {
                ...
                this.tables.map(table => {
                    ...
                    if(table==='settings'){
                      storeObjects['store' + table].createIndex('device', ['device']);
                    }
                });
    ...
    getItems(objectStore, column='enabled, deleted', query=[1,0]): Promise<any> {
        ...
        const promise = new Promise<Plant | Strain | Company | Dose | Calendar | any>(resolve => {
          if(query.length > 0) {
            ...
            queryExecute.onerror = e => { console.log(e); };
          } else {
