...
private createDb(): Promise<void> {
    ...
    return new Promise(resolve => {
        ...
        openRequest.onupgradeneeded = event => {
            const target: any = event.target, db = target.result, storeObjects: any[] = [];
            this.tables.map(table => {
                storeObjects[<any>`store${table}`] = db.createObjectStore(table, {keyPath: 'id', autoIncrement: true});
                storeObjects[<any>`store${table}`].createIndex('id', ['id']);
                storeObjects[<any>`store${table}`].createIndex('synced', ['synced'], { unique: false });
                storeObjects[<any>`store${table}`].createIndex('deleted', ['deleted'], { unique: false });
                storeObjects[<any>`store${table}`].createIndex('enabled, deleted', ['enabled', 'deleted']);
                if (table === 'doses') {
                    storeObjects[<any>`store${table}`].createIndex('enabled, deleted, id_calendar', ['enabled', 'deleted', 'id_calendar']);
                }
            });
        ...
        openRequest.onsuccess = (event) => {
            ...
            this.db.onerror = (event:any) => { console.error('[DB]: error createDb: ' + event); };
...
async initService(forceLoading= false): Promise<void> {
    const networkStatus = this.api.networkService.status.value, date = new Date(), now = Date.now(), lastUpdate :any = [], promises = [];
        ...
        data.map((data: any) => {
            ...
            if (res.items.length) {
                ...
                const promises = res.items.map((row: any) => {
                    ...
                    tx.oncomplete = (e:any) => {
                        if (lastUpdate) {
                            localStorage.setItem(this.appSettings.appName + '_' + table, String(lastUpdate));
                        }
              ...
              this.getItemsToBeSynced(table).then((items) => {
                  if (items.length) {
                      ...
                      items.map((item: any) => {
                          ...
                          saveItem.push(item);
                          this.putItems(table, saveItem).then(() => { resolve(true); });
                      });
                  } else { resolve(false); }
              });
          });
          ...
          this.tables.map((table) => {
              this.getItemsToBeRemoved(table).then((items) => {
                  if (items.length) {
                      ...
                      items.map((item: any) => {
                          this.deleteItem(table, item).then(() => { resolve(true); });
                      });
                  } else { resolve(false); }
              });
          });
