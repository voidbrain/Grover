...
@Component({
  selector: 'app-detail',
  standalone: true,
  imports: [RouterLink, ChartComponent, FormsModule],
...
export class CalendarsDetailComponent {
  ...
  getNextPos() {
    return (this.relatedPhases.length ? Math.max.apply(Math, this.relatedPhases.map(function(o: any) { return o.pos + 1; })) : 1) ;
  }
  ...
  saveForm() {
    ...
    saveItem[<any>'phases'].forEach((phase:any, index:any)=>{
        saveItem[<any>'phases'][index] = (({ id, pos, duration }) => ({ id, pos, duration }))(phase);
    });
