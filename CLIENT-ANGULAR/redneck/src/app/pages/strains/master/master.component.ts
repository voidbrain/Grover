...
@Component({
  ...
  imports: [
    ...
    ChartComponent, IonButton, IonButtons, IonContent, IonHeader, IonIcon, IonItem, IonItemOption, IonItemOptions, IonItemSliding, IonLabel, IonList, IonMenu, IonMenuToggle, IonRefresher, IonRefresherContent, IonTitle, IonToolbar
  ],
  ...
  private getItems() {
    ...
    Promise.all([itemsP, calendarsP, dosesP, strainsP]).then(([items, calendars, doses, strains]) => {
        ...
        items.map((item: any) => {
            ...
            item.dose = doses.find((singleDose: any) => {singleDose.id == dose.id_dose; });
