...
import { EventEmitter, OperatingModes, ServerCommands, ServerPages, DevicesStatus } from './app/services/settings/enums';
...
class Main {
  ...
  async main() {
    try {
      ...
      this.room = new RoomComponent(this.serialNumber.sn, this.db, this.api, this.settings) as unknown as RoomObject;
  ...
  async setMainSchedule() {
    try {
      this.scheduledCrons = await this.db.getItems('system_schedule') as unknown as any[];
      const scheduleArr: CronJobInterface[] = this.scheduledCrons.map(systemScheduleRow => ({
        ...
        switch(page) {
          case `/${ServerPages.actuators}`:
            ...
            if (action && id && terminalType) {
              ...
              const terminal: any = await this.db.getItem(terminalType + 's_list', +id, 'id');
              const parentLocation: LocationInterface = await this.db.getItem('locations', +terminal.locationId, 'id');
              ...
              res.write(JSON.stringify({ error: `Error ${action}, ${id}, ${terminalType}` }));
            }
            break;
