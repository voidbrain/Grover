...
export class AiService {
  ...
  async init() {
    ...
    this.fertilizerModel = await tf.loadLayersModel(
      "file://" + this.rootDir + "/data/ai/history-model/model.json",
    );
    this.ecPhModel = await tf.loadLayersModel(
      "file://" + this.rootDir + "/data/ai/history-model/model.json",
    );
  ...
  getDoses(req) {
    try {
      ...
      const inputTensor = tf.tensor2d([
        [+waterLevel, +plantAge, +desiredEC, +desiredPH],
      ]);
  ...
  async trainEcPhModel() {
    ...
    // Save the model
    await this.ecPhModelTrain.save(
      "file://" + this.rootDir + "/data/ec-ph-levels",
    );
  }
  ...
  async trainDosesModel() {
    ...
    // Save the model
    await this.dosesModelTrain.save(
      "file://" + this.rootDir + "/data/history-model",
    );
  }
}
