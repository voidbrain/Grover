...
export class DbService {
  ...
  async load(): Promise<any> {
    ...
    return new Promise<void>(async (resolve, reject) => {
      ...
      self
        .initService(resetDb ? resetDb : forceLoading)
        .then(async () => {
  ...
  async initService(forceLoading = false): Promise<void> {
    ...
    const lastGlobalUpdate =
      self.localStorage.getItem(
        self.settings.getAppName() + "_lastglobalupdate",
      ) || now.date() - 1;
    ...
    Promise.all(
      self.tables.map(async (table) => {
        lastUpdate[table] = self.localStorage.getItem(
          self.settings.getAppName() + "_" + table,
        );
        if (self.debug) {
  ...
  async loadData(table, lastUpdate): Promise<any> {
    ...
    return new Promise(async (resolve, reject) => {
      const res = self.api
        .get(table, lastUpdate, "read", this.serialNumber)
        .then(async (res) => {
  ...
  async getItem(
    ...
    const promise = new Promise<
      LocationInterface | RoomInterface | PotInterface | any
    >((resolve) => {
      if (value) {
  ...
  async getItems(
    ...
    const promise = new Promise<LocationInterface[] | RoomInterface[]>(
      (resolve) => {
  ...
  async putItem(table, item: any): Promise<void> {
    ...
    const promise = new Promise<LocationInterface[] | RoomInterface[]>(
      (resolve, reject) => {
        ...
        self.api
          .post(endpoint, lastUpdate, action, item, self.serialNumber)
          .then((response: any) => {
        ...
        self.api
          ...
          .then((response: any) => {
            if (response) {
              ...
              console.log(
                "[DB]: logItem API POST response undefined",
                response,
              );
            }
          });
  ...
  async deleteItem(objectStore, itemToDelete: any): Promise<void> {
    const self = this;
    this.api.delete(objectStore, itemToDelete).then((item: any) => {
  ...
  getItemsToBeSynced(table): Promise<any> {
    const self = this;
    const promise = new Promise<LocationInterface[] | RoomInterface[] | any[]>(
      (resolve) => {
