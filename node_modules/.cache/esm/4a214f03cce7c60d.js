let Pot,Room;_cce‍.w("./app/envoirement/pot/pot",[["default",["Pot"],function(v){Pot=v}]]);_cce‍.w("./app/envoirement/room/room",[["default",["Room"],function(v){Room=v}]]);/**
 ...
 */
  ...
  serverCallRemote(page, action=null){
    ...
    https.get(self.remoteServer+page + '?action=' + action, (resp) => {
      ...
      resp.on('end', () => {
        _cce‍.g.console.log(JSON.parse(data));
      });
    }).on("error", (err) => {
      _cce‍.g.console.log("Error: " + err.message);
    });
  }
  serverStart(){
    const self = this;
