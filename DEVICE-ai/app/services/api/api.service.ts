...
export class ApiService {
  ...
  async httpsGet({ ...options }: { hostname: string; path: string }) {
    return new Promise((resolve, reject) => {
      const url = `${options.hostname}/${options.path}`;
      const isLocalhost = options.hostname.includes("localhost") || options.hostname.includes("127.0.0.1");
  ...
  async httpsPost(path: string, body: unknown) {
    return new Promise((resolve, reject) => {
      axios({
        method: "POST",
        url: path,
  ...
  async httpsDelete({ body, ...options }: { body?: string; hostname: string; path: string; headers?: Record<string, string> }) {
    return new Promise((resolve, reject) => {
