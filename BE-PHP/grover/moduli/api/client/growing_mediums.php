...
$default_table = $config['db_pfx']."mediums";
...
switch ($method) {
	...
	// 	if($el->id){
	// 		$item = $db->where("id",$el->id)->update($default_table,$item);
	// 	}else{
	// 		$item = $db->insert($default_table,$item);
	// 	}
	// 	$output = (array)$item;
	// 	break;
    case 'POST':
	...
	// case 'DELETE':
	// 	$id = $_GET["id"];
	// 	$item = $db->where("id",$el->id)->delete($default_table,$item);
	// 	$data = array(
	// 		'result' => $item,
	// 		'id' => $id
	// 	);
