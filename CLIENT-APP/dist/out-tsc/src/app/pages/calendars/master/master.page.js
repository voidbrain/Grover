...
var MasterPage = /** @class */ (function () {
    ...
    MasterPage.prototype.getItems = function () {
        ...
        Promise.all([itemsP, dosesP]).then(function (_a) {
            var items = _a[0], doses = _a[1];
            items.map(function (item, i) {
                item.doses = doses.filter(function (el) { return el.id_calendar == item.id; });
                var valuesArr = [];
                item.doses.forEach(function (el) {
                    valuesArr.push({
                        data: [Math.floor(el.duration / 7)],
