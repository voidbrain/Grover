...
class RoomNutrientRefillComponent {
  ...
  public async backward (pump) {
    return new Promise(resolve => {
      this[`primaryNutrientPump${pump}`].digitalWrite(this[`pin1${pump}`], this[`primaryNutrientPump${pump}`].LOW);
      this[`primaryNutrientPump${pump}`].digitalWrite(this[`pin2${pump}`], this[`primaryNutrientPump${pump}`].HIGH);
      resolve(true);
    });
  };

  public async stop (pump) {
    console.log(`[ROOM-Nutrient-REFILL]: stop ${pump}`);
    return new Promise(resolve => {
      this[`primaryNutrientPump${pump}`].digitalWrite(this[`primaryNutrientPump${pump}`], this[`primaryNutrientPump${pump}`].LOW);
      this[`primaryNutrientPump${pump}`].digitalWrite(this[`primaryNutrientPump${pump}`], this[`primaryNutrientPump${pump}`].LOW);
      resolve(true);
    });
  };
