      ...
      if(operatingMode >= systemOperatingMode) {
        ...
        switch(owner){
          case Owner.user: // manual action
            console.log("[ROOM-Bloom-REFILL]: RUN_PH manual", job);
          ...
          case Owner.schedule: // scheduled action
            console.log("[ROOM-Bloom-REFILL]: RUN_PH scheduled", job);
            if (self.settings.getLogMode() === true) { 
              await self.db.logItem('workers_log', job); 
              resolve;
            }
        ...
        console.log(`[ROOM-Bloom-REFILL]: RUN_PH operatingMode insufficient level (probe: ${operatingMode} system: ${systemOperatingMode})`);
      }
    });
  }
